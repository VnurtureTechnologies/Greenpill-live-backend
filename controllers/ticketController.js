        ...
        .then( async(response) => {
            if(response.data().performaInvoice) {
                ...
                req.files.forEach(async(file) => {
                    if(file.fieldname == 'ticket_files_performa') {
                        const performaUrl = await helpers.uploadTicketsPdf(req.files[0]);
                        update_data['performaInvoice'] = performaUrl;
                    } else if (file.fieldname == 'ticket_files_quotation') {
                        const quotationUrl = await helpers.uploadTicketsPdf(req.files[1]);
